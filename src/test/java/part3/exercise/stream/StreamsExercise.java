...
public class StreamsExercise {
    ...
    public void indexPersonsByEmployer1() {
        ...
        final Map<String, List<PersonEmployer>> index = employees
                .stream()
                .flatMap(s -> s.getJobHistory().stream().map(e -> new PersonEmployer(s.getPerson(), e.getEmployer())))
    ...
    public void indexPersonsByEmployer2() {
        ...
        final Map<String, List<Person>> index = employees.stream()
                .flatMap(s -> s.getJobHistory().stream()
                        .map(e -> new PersonEmployer(s.getPerson(), e.getEmployer())))
                .collect(Collectors.toMap(
                        PersonEmployer::getEmployer,
                        person -> {
                            final ArrayList<Person> persons = new ArrayList<>();
                            persons.add(person.getPerson());
                            return persons;
                        },
                        (p1, p2) -> {
                            p1.addAll(p2);
    ...
    private PersonDuration sumAllPersonDurations(Employee e) {
        return new PersonDuration(
                e.getPerson(), e.getJobHistory().stream()
                .mapToInt(JobHistoryEntry::getDuration)
                .sum());
    }
    ...
    public void getSumPersonDuration() {
        ...
        final Map<Person, Integer> personDuration = employees
                .stream()
                .collect(Collectors.toMap(Employee::getPerson, e -> sumAllPersonDurations(e).duration));
        ...
        final Map<String, PersonPositionDuration> coolestPersonByPosition = employees
                ...
                .flatMap(e -> e.getJobHistory()
                        ...
                        .map(j -> new PersonPositionDuration(
                                ...
                                j.getPosition(),
                                j.getDuration())))
                .collect(Collectors.toMap(PersonPositionDuration::getPosition, Function.identity(), BinaryOperator.maxBy(Comparator.comparingInt(PersonPositionDuration::getDuration))));
